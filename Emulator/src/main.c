...
byte readPort(byte port) {
  ...
  ret = 255;
  if (cidelsa == 'Y') {
    if (port == 1) return cid1^0xff;
    if (port == 2) return cid2^0xff;
    if (port == 4) return cid4^0xff;
    }
    ...
    if (event.window == hexWindow) keypadEvent(event);
      ...
      if (event.type == EVENT_KEY_RELEASE) {
        if (cidelsa == 'Y') {
          ...
          if (event.d1 == KEY_LEFT) cid1 |= 8;
          if (event.d1 == KEY_RIGHT) cid1 |= 16;
      ...
      if (event.type == EVENT_KEY_PRESS) {
        if (cidelsa == 'Y') {
          ...
          if (event.d1 == KEY_LEFT) cid1 &= 0xf7;
          if (event.d1 == KEY_RIGHT) cid1 &= 0xef;
